...
void Number::checkType() {
    type = std::make_shared<Type>("number", 1);
}

void String::checkType() {
    type = std::make_shared<Type>("string", 1);
}

void Bool::checkType(){
    type = std::make_shared<Type>("boolean", 1);
}
...
void Indexing::checkType() {
    Object->checkType();
    Index->checkType();
    if(Index->type != std::make_shared<Type>("number")){
        LogError("Index must be a number");
        exit(1);
    }
    type = std::make_shared<Type>(*(Object->type));
    type->size = 1;
}
...
void IfStatement::checkType() {
    ...
    Symbols.DestroyScope();
    type = std::make_shared<Type>("void");
}

void ForLoop::checkType() {
    Start->checkType();
    if(Start->type != std::make_shared<Type>("number")){
        LogError("Start must be a number");
        exit(1);
    }
    Symbols.CreateScope();
    Symbols.CreateVariable(VariableName, std::make_shared<Type>("number"));
    Condition->checkType(); // TODO: check if condition is boolean
    Step->checkType();
    if(Step->type != std::make_shared<Type>("number")){
...
void Function::checkType(){
    ...
    Symbols.DestroyScope();
    // type = std::make_shared<Type>("void"); // TODO: make the type of this node irrelevant in codegenning, so we can correctly save the type of this node
}
